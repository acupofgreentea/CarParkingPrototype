...
public class OffScreenIndicator : MonoBehaviour
{
    ...
    void DrawIndicators()
    {
        foreach(Target target in targets)
        {
            ...
            bool isTargetVisible = target.IsVisible;
            ...
            {
                screenPosition.z = 0;
                indicator = GetIndicator(ref target.indicator, IndicatorType.BOX, isTargetVisible); // Gets the box indicator from the pool.
            }
            else if(target.NeedArrowIndicator && !isTargetVisible)
            {
                float angle = float.MinValue;
                OffScreenIndicatorCore.GetArrowIndicatorPositionAndAngle(ref screenPosition, ref angle, screenCentre, screenBounds);
                indicator = GetIndicator(ref target.indicator, IndicatorType.ARROW, isTargetVisible); // Gets the arrow indicator from the pool.
                indicator.transform.rotation = Quaternion.Euler(0, 0, angle * Mathf.Rad2Deg); // Sets the rotation for the arrow indicator.
            }
            else if(target.NeedArrowIndicator && isTargetVisible)
            {
                indicator = GetIndicator(ref target.indicator, IndicatorType.ARROW, isTargetVisible);
    ...
    /// <returns></returns>
    private Indicator GetIndicator(ref Indicator indicator, IndicatorType type, bool isTargetVisible)
    {
        if(indicator != null)
        {
